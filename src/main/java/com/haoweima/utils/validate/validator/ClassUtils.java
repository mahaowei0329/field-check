        ...
        try {
            ...
            while (dirs.hasMoreElements()) {
                ...
                if ("file".equals(protocol)) {
                    ...
                    String packageName = filePath.substring(1).replace("/", File.separator);
...
//                }
    ...
    public static void findAndAddClassesInPackageByFile(String packageName, String packagePath, final boolean recursive, Set<Class<?>> classes) {
        ...
        for (File file : dirfiles) {
            // 如果是目录 则继续扫描
            if (file.isDirectory()) {
                findAndAddClassesInPackageByFile(packageName, file.getAbsolutePath(), recursive, classes);
            } else {
                // 如果是java类文件 去掉后面的.class 只留下类名
                String fileName = file.getPath();
                String className = fileName.replace(packageName, "").replace(File.separator, ".");
                try {
                    // 添加到集合中去
                    // classes.add(Class.forName(packageName + '.' + className));
    ...
    public static Set<Class<?>> getClassesByAnnotation(String annotationName){
        ...
        for(Class<?> clz : classSet){
            ...
            for(Annotation temp : annos){
                if(temp.annotationType().getSimpleName().equals(annotationName)){
                    result.add(clz);
                }
            }
        }
